...
impl Command for RtCommand {
    ...
    fn execute(&self, args: &CommandLineArguments) -> Result<(), CommandExecutionError> {
...

        {
            ...
            thread::spawn(move || {
                if let Err(err) = start_gnirehtet(serial.as_ref(), dns_servers.as_ref()) {
...
fn exec_adb<S: Into<String>>(
    ...
    let exit_status = process::Command::new("adb")
        .args(&adb_args[..])
        .status()?;
...
fn is_gnirehtet_installed(serial: Option<&String>) -> Result<bool, CommandExecutionError> {
    let args = create_adb_args(serial, vec!["shell", "pm", "list", "packages", "com.genymobile.gnirehtet"]);
    let output = process::Command::new("adb").args(&args[..]).output()?;
    // empty output when not found
    Ok(!output.stdout.is_empty())
}

fn start_gnirehtet(serial: Option<&String>, dns_servers: Option<&String>) -> Result<(), CommandExecutionError> {
    info!("Starting gnirehtet...");
    exec_adb(serial, vec!["reverse", "tcp:31416", "tcp:31416"])?;

    let mut adb_args = vec!["shell", "am", "startservice", "-a", "com.genymobile.gnirehtet.START"];
...
fn stop_gnirehtet(serial: Option<&String>) -> Result<(), CommandExecutionError> {
    ...
    exec_adb(serial, vec!["shell", "am", "startservice", "-a", "com.genymobile.gnirehtet.STOP"])
}
